...
export default {
  async fetch(request, env, ctx) {
    ...
    if (response.status === 403) {
      console.error("Strava returned 403: Cookies likely expired.");
    ...
    // If there are any other errors, return now before we get to caching
    if (!response.ok) {
      return response;
    }
    ...
    // TODO: I don't know why this is necessary but AI insists that it is
